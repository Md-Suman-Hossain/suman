...
ADMIN_ID = 7878473793
# --- UPDATED API BASE URMino_SMS_boTL AND KEY ---
...
PREMIUM_FLAGS = {
    ...
    "new zealand": "5913640044937089340", "netherlands": "5913367645226275100", "nepal": "5913496520014958723",
    ...
    "liechtenstein": "5911166650580734660", "libyan arab jamahiriya": "5911236989260140996", "libya": "5911236989260140996",
    "liberia": "5913324167272337727", "kiribati": "5911294443037660118", "kosovo": "5911433681582429010",
    "kuwait": "5913290705182134003", "kyrgyzstan": "5911202161370337549", "lao people's democratic republic": "5913718526874489279",
    ...
    "croatia": "5913692684056269311", "costa rica": "5911261745451635030", "congo": "5911338788574990168",
    "congo, the democratic republic of the": "5913770362834783827", "democratic republic of the congo": "5913770362834783827",
    "comoros": "5911338582416560604", "cambodia": "5913699998385573485", "cameroon": "5911172109484167745",
    "canada": "5913623736946265914", "cape verde": "5913571501554012193", "central african republic": "5913443245240619222",
    ...
    "algeria": "5913782968563800236", "albania": "5911357458797826163", "afghanistan": "5913492040364068694",
    "zimbabwe": "5911092502265336396", "cuba": "5431551436502611633", "korea, democratic people's republic of": "5434142701941437163",
    ...
    with db_lock:
        ...
        default_data = {
            ...
            "texts": {
                ...
                "support_link": "https://t.me/NETBOLDNETMAIR0",
                "otp_group_link": "https://t.me/+31eV11IT7WQzMjI9",
                "main_channel_link": "https://t.me/MinoXofficial0",
                "renge_group_link": "https://t.me/voltxsmsrenge",
...
def get_updated_number_markup(chat_id):
    with active_sessions_lock:
        ...
        for num in numbers:
            ...
            markup.add(
                ibtn(f"- {num}", copy_text_str=num, custom_emoji_id=emoji_id)
            )
        ...
        data = load_data()
        markup.add(
            ibtn("Otp Group", url=data.get("texts", {}).get("otp_group_link", "https://t.me/+sumanotp653"), style="primary", custom_emoji_id=get_emoji_id("emj_otp_group")),
            ibtn("Main Channel", url=data.get("texts", {}).get("main_channel_link", "https://t.me/MinoXofficial0"), style="primary", custom_emoji_id=get_emoji_id("emj_link"))
        )
        return markup
    ...
    if ranges_list:
        ...
        while len(fetched_numbers) < max_nums and attempts < max_attempts:
            ...
            try:
                ...
                res = http_session.post(f"{BASE_URL}/getnum", json=payload, headers=headers, timeout=8).json()
        ...
        if file_nums:
            ...
            # আপলোড করা নাম্বারগুলো যখন বোট থেকে সিলেক্ট করা হচ্ছে, তখন প্যানেলে পাঠানোর লজিক।
            # সাধারণত এই প্যানেলগুলোতে /getnum এন্ডপয়েন্টে ম্যানুয়ালি নাম্বার পুশ করা যায়।
            for f_num in fetched_numbers:
                try:
                    sync_headers = {'mauthapi': api_key, 'Content-Type': 'application/json'}
                    # প্যানেলকে জানানো হচ্ছে যে এই নাম্বারের জন্য ওটিপি রিকোয়েস্ট আসছে
                    # Note: /add-manual-num একটি জেনেরিক নাম, আপনার প্যানেলের আসল নাম আলাদা হতে পারে
                    sync_payload = {"number": f_num, "sync": "true"}
                    http_session.post(f"{BASE_URL}/getnum", json=sync_payload, headers=sync_headers, timeout=5)
                except: pass
        else:
...
def poll_otp_real(chat_id, phone_number, service_info, msg_id):
    ...
    while time.time() - start_time < timeout:
        ...
        try:
            res = http_session.get(f"{BASE_URL}/success-otp", headers=headers, timeout=10).json()
            meta_part = res.get("meta") or {}
            code = meta_part.get("code")
            if code == 200 or str(code) == "200":
                data_part = res.get("data") or {}
                ...
                for otp_entry in otps_list:
                    ...
                    if entry_num == target_num:
                        ...
                        if otp_signature not in processed_otps:
                            ...
                            user_msg = (
                                f"{get_emoji_tag('emj_done')} <b>OTP RECEIVED SUCCESSFULLY!</b>\n\n"
                                f"{get_emoji_tag('emj_country')} <b>Country:</b> {escape_html(service_info['cnt_name'])} {premium_flag}\n"
                                f"{get_emoji_tag('emj_number')} <b>Number:</b> <code>{phone_number}</code>\n"
                                f"{get_emoji_tag('emj_otp_received')} <b>OTP Code:</b> <code>{escape_html(otp_code)}</code>\n\n"
                                f"{get_emoji_tag('emj_message')} <b>Full SMS:</b>\n<code>{escape_html(full_sms)}</code>"
                            )

                            group_msg = (
                                f"{premium_flag} <b>#{escape_html(service_info['cnt_name'])} {srv_emoji} OTP SUCCESS</b>\n\n"
                                f"<b>Number:</b> <code>{mask_number(phone_number)}</code>\n"
                                f"<b>OTP Code:</b> <code>{escape_html(otp_code)}</code>\n"
                                f"<b>Full SMS:</b> <code>{escape_html(full_sms)}</code>"
                            )
                            
                            markup = InlineKeyboardMarkup(row_width=2)
                            markup.add(
                                ibtn(f" {otp_code}", copy_text_str=otp_code, style="primary", custom_emoji_id=get_emoji_id("emj_otp_received")),
                                ibtn(" Full SMS", copy_text_str=full_sms, style="primary", custom_emoji_id=get_emoji_id("emj_message"))
                            )
                            markup.add(
                                ibtn(" NUMBER BOT", url=f"https://t.me/{get_bot_username()}", style="success", custom_emoji_id=get_emoji_id("emj_link")),
                                ibtn(" MAIN CHANNEL", url=data.get("texts", {}).get("main_channel_link", "https://t.me/MinoXofficial0"), style="success", custom_emoji_id=get_emoji_id("emj_link"))
                            )
                            
                            try: bot.send_message(chat_id, user_msg, parse_mode="HTML", reply_markup=markup)
                            except: pass
                            
                            for grp in data.get("forward_groups",[]):
                                try:
                                    bot.send_message(grp, group_msg, parse_mode="HTML", reply_markup=markup)
                                except: pass
        except: pass
        time.sleep(4)
...
def check_api_provider_balance(chat_id, message_id=None):
    ...
    try:
        res = http_session.get(f"{BASE_URL}/balance", headers=headers, timeout=10)
        if res.status_code == 200:
            ...
            api_msg = (
                ...
                f"▫️ <b>Endpoint URL:</b> <code>2oo9.cloud</code>\n"
