...
PREMIUM_FLAGS = {
    ...
    "liechtenstein": "5911166650580734660", "libyan arab jamahiriya": "5911236989260140996", "libya": "5911236989260140996", "liberia": "5913324167272337727", "kiribati": "5911294443037660118", "kosovo": "5911433681582429010",
                ...
                for otp_entry in otps_list:
                    ...
                    if entry_num == target_num:
                        ...
                        if otp_signature not in processed_otps:
                            ...
                            # --- UPDATED MESSAGE DESIGN AS PER SCREENSHOT ---
                            user_msg = (
                                f"{premium_flag} <b>Number:</b> <code>{phone_number}</code>\n"
                                f"🏆 <b>Reward:</b> <code>{bonus}</code>$\n"
                                f"🔑 <b>OTP:</b> <code>{escape_html(otp_code)}</code>\n"
                                f"💬 <b>Full SMS</b>\n"
                                f"<code>{escape_html(full_sms)}</code>"
                            )
    ...
    try:
        with open(file_path, "w", newline="", encoding="utf-8-sig") as f:
            ...
            for u in users:
                ...
                status = "Banned" if u in data.get("banned_users", []).get("users", []) else "Active"
