...
PREMIUM_FLAGS = {
    ...
    "georgia": "5913434771270144023", "gambia": "5913657267755945883", "gabon": "5911037896051137264", "france": "5913605586414473124", "finland": "5911041344909873378", "fiji": "5911393832875856716",
...
# ============================================
# --- REAL OTP POLLING & FORWARDING (UI SCREENSHOT SYNC) ---
# ============================================
                ...
                for otp_entry in otps_list:
                    ...
                    if entry_num == target_num:
                        ...
                        if otp_signature not in processed_otps:
                            ...
                            bonus_text = data.get("settings", {}).get("otp_bonus", 0.001)
                            
                            # --- SCREENSHOT STYLE OTP MESSAGE ---
                            user_msg = (
                                f"{premium_flag} <b>Number:</b> <code>{phone_number}</code>\n"
                                f"🏆 <b>Reward:</b> {bonus_text}$\n"
                                f"🔑 <b>OTP:</b> <code>{otp_code}</code>\n"
                                f"💬 <b>Full SMS</b>\n"
                                f"<code>{escape_html(full_sms)}</code>"
                            )

                            # --- SCREENSHOT STYLE BUTTONS ---
                            markup = InlineKeyboardMarkup(row_width=1)
                            srv_id = service_info.get("srv_id", "custom")
                            cnt_id = service_info.get("cnt_id", "custom")
                            
                            markup.add(
                                ibtn("🔄 Change Number", callback_data=f"chg_r|{srv_id}|{cnt_id}", style="danger"),
                                ibtn("🌍 Change Country", callback_data=f"usr_s|{srv_id}", style="success"),
                                ibtn("🔔 Otp Group", url=data.get("texts", {}).get("otp_group_link", "https://t.me/+sumanotp653"), style="primary")
                            )
                            ...
                            for grp in data.get("forward_groups",[]):
                                try:
                                    bot.send_message(grp, group_msg, parse_mode="HTML")
                                except: pass
        except: pass
        time.sleep(4)
...
def show_support(chat_id):
    ...
    sup_lnk = texts.get("support_link", "https://t.me/sumannamber")
